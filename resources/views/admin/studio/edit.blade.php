...
@section('page-subtitle', 'Update informasi studio')
            ...
            <form action="{{ route('admin.studio.update', $studio->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-5">
                    ...
                    <textarea name="deskripsi" rows="4"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent">{{ old('deskripsi', $studio->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <!-- Upload Gambar Baru -->
                <div class="mb-5">
                    <label class="block text-gray-700 font-medium mb-2">Upload Gambar Baru</label>
                    <input type="file" name="gambar" accept="image/*" id="gambar"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent">
                    <p class="text-sm text-gray-500 mt-1">Format: JPG, PNG (Max: 2MB) - Kosongkan jika tidak ingin mengubah
                    </p>
                    @error('gambar')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
